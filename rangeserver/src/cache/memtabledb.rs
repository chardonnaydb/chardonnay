...
impl MemTableDB {
    async fn upsert_or_delete(
        ...
        if self.memtables[self.mut_memtable_idx].size_in_bytes + entry_size
            > self.cache_options.write_buffer_size
        {
            ...
            if self.memtables[new_id].status != MemTableStatus::FLUSHED {
                println!(
                    "No free memtables {} {:?}",
                    new_id, self.memtables[new_id].status
                );
        ...
        self.memtables[self.mut_memtable_idx].latest_epoch =
            max(self.memtables[self.mut_memtable_idx].latest_epoch, epoch);
...
impl Cache for MemTableDB {
    async fn new(cache_options: CacheOptions) -> Self {
    ...
    async fn delete(&mut self, key: bytes::Bytes, epoch: u64) -> Result<(), Error> {
    ...
    async fn get(
        ...
        for i in 0..self.cache_options.num_write_buffers {
            let idx = (self.mut_memtable_idx + self.cache_options.num_write_buffers - i)
                % self.cache_options.num_write_buffers; // mut_id is usize, should not become negative
    ...
    async fn clear(&mut self, epoch: u64) -> Result<(), Error> {
        ...
        for i in 1..self.cache_options.num_write_buffers {
            let id = (self.mut_memtable_idx + self.cache_options.num_write_buffers - i)
                % self.cache_options.num_write_buffers; // mut_id is usize, should not become negative
            if self.memtables[id].status == MemTableStatus::IMMUTABLE
                && epoch >= self.memtables[id].latest_epoch
            {
                self.memtables[id].status = MemTableStatus::FLUSHED;
                self.memtables[id].size_in_bytes = 0;
                self.memtables[id].entries.clear();
                println!(
                    "Changing memtable {} state from IMMUTABLE to {:?}",
                    id, self.memtables[id].status
                );
            }
        }
        Ok(())
    ...
    async fn register_gc_callback(&mut self, cb: GCCallback) -> Result<(), Error> {
...
pub mod tests {
    ...
    async fn fill(mt_db: &Arc<RwLock<MemTableDB>>, num_keys: u64, clear: bool) {
        ...
        for i in 0..num_keys {
            ...
            let epoch = i;
            println!(
                "FILL: insert key {:?} value_size {:?} epoch {:?}",
                key,
                value.len(),
                epoch
            );
    ...
    async fn insert_get() {
        ...
        let num_keys =
            mt_db.read().await.cache_options.write_buffer_size / for_testing::KV_SIZE - 1;
    ...
    async fn get_immutable_memtable() {
        ...
        // fill two memtables
        let num_keys =
            2 * (mt_db.read().await.cache_options.write_buffer_size / for_testing::KV_SIZE - 1);
        fill(&mt_db, num_keys, false).await;
        println!("Fill complete >>>>>>!");
    ...
    async fn clear() {
        ...
        // fill memtables
        let num_keys =
            100 * (mt_db.read().await.cache_options.write_buffer_size / for_testing::KV_SIZE - 1);
                ...
                for i in 0..10000 {
                    ...
                    match rd_td.values.get(&key) {
                        ...
                        Some(v) => {
                            ...
                            assert!(
                                ep <= rand_epoch,
                                "get returned wrong epoch! expected {:?} got {:?}",
                                rand_epoch,
                                ep
                            );
                        }
